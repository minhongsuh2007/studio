...
export const translations: Translations = {
  ...
  ja: {
    ...
    cardDescription: 'PNG、JPG、GIF、またはWEBPファイルを追加します。星のパターン伝播アルゴリズムを使用して画像が整列されます。最良の結果を得るには、最初の画像で参照星を手動で選択してください。',
    ...
    tutorialStep1Title: "ステップ1：ライトフレームをアップロード",
    tutorialStep1Content: "「ここに画像をドラッグ＆ドロップ」エリアをクリックするか、主な天体写真（「ライト」）をそこにドロップします。これらはターゲットオブジェクトのメイン画像です。一度に複数のファイルをアップロードできます。アップロード後、アプリは各画像を自動的に分析して星を検出します。",
    tutorialStep2Title: "ステップ2：（オプション）手動で星を選択",
    tutorialStep2Content: "最高のアライメント精度を得るために、手動でプロセスをガイドできます。キューの最初の画像は「参照画像」です。その上で「星を選択」をクリックします。メインプレビューウィンドウで、星の近くをクリックしてアライメント用に手動で選択できるようになります。赤い円が表示されます。最低2つの星が必要です。これにより、アプリにアライメントに使用するポイントを正確に伝えることができます。",
    tutorialStep3Title: "ステップ3：スタッキングオプションを選択",
    tutorialStep3Content: "左側のパネルで、画像のスタック方法を設定できます。「スタッキングモード」（平均、中央値、シグマクリップ）は、ピクセル値の結合方法を決定します。「出力形式」では、PNG（可逆）とJPG（ファイルサイズが小さい）のどちらかを選択できます。",
    tutorialStep4Title: "ステップ4：画像をスタック",
    tutorialStep4Content: "画像がアップロードされ、設定に満足したら、「画像をスタック」ボタンをクリックします。アプリは各画像を参照フレームに整列させ、それらを結合します。プログレスバーでステータスが表示されます。",
